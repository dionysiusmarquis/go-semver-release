...
func (w *commitWalker) Next() (*object.Commit, error) {
	...
	err := parents.ForEach(func(p *object.Commit) error {
		...
		// If there is more than one parent insert all parent commit branch commits by prepending them to the queue
		...
		if i != 0 {
			...
			pp, err := p.Parent(0)
			if err != nil {
				return fmt.Errorf("fetching parent: %w", err)
			}
			w.queue = append([]*queueEntry{{current: pp, end: end}}, w.queue...)
		} else {
			entry.current = p
		}
		i++
